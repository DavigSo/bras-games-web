...
const cartController = {
  addToCart: async (req, res) => {
    try {
      ...
      if (!cart) {
        ...
        cart = await Cart.create({ user: userId, games: [{ game: gameId, quantity: 1 }] });
        user.cart = cart._id;
        await user.save();
        ...
        const existingGame = cart.games.find(item => item.game.equals(gameId));

        if (existingGame) {
          existingGame.quantity += 1;
          console.log('Quantidade do jogo incrementada:', existingGame.quantity);
        } else {
          cart.games.push({ game: gameId, quantity: 1 });
          console.log('Jogo adicionado com sucesso:', cart.games[cart.games.length - 1]);
        }
      }
      ...
      res.status(500).json({ error: 'Erro interno ao adicionar jogo ao carrinho' });
    }
  },
  ...
  updateCartItem: async (req, res) => {
    try {
      ...
      if (!gameItem) {
        return res.status(404).json({ error: 'Item do jogo não encontrado no carrinho' });
      }

      gameItem.quantity = quantity;

      await cart.save();
      res.status(200).json({ message: 'Quantidade do item do jogo atualizada com sucesso' });
    } catch (error) {
      console.error('Erro ao atualizar item do jogo no carrinho:', error);
      res.status(500).json({ error: 'Erro interno ao atualizar item do jogo no carrinho' });
    }
  },
  ...
  removeCartItem: async (req, res) => {
    try {
      ...
      await cart.save();
      res.status(200).json({ message: 'Item do jogo removido do carrinho com sucesso' });
    } catch (error) {
      console.error('Erro ao remover item do jogo do carrinho:', error);
      res.status(500).json({ error: 'Erro interno ao remover item do jogo do carrinho' });
    }
  },
  getCartDetails: async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Erro interno ao obter detalhes do carrinho' });
    }
  }
};
